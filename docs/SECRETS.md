...
5. **Сторонние API-ключи** (DaData, почта, эквайринг) — не вставлять в чаты, тикеты и скриншоты; при утечке **сразу перевыпустить** в кабинете провайдера
...
- опционально (контейнер **web**): подсказки адресов DaData — `DADATA_TOKEN` или `DADATA_API_KEY` (тот же API-ключ из кабинета); при необходимости пара `DADATA_SECRET` / `DADATA_SECRET_KEY`. Ключи только на сервере, в браузер не попадают.
...
- Ключи DaData и других внешних API (только `process.env` на сервере, не `NEXT_PUBLIC_*`)
