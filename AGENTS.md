...
- **Ветки WMS:** ориентир — **`dev`** (`git pull origin dev`). Старые длинные ветки с несмёрженным бэкендом держим как **архив на GitHub** (например `origin/feat/wms-invoice-agx`); новая задача — **ветка от свежего `dev`**, без раздувания старых `feat/*` без rebase/merge.

- **Ветки WMS (чтобы не путать репозиторий):** единый ориентир для выкатки в staging — **`dev`** (`git checkout dev && git pull origin dev`). Старую длинную ветку с несмёрженным бэкендом **не удаляем с GitHub** — она остаётся **архивом** (например `origin/feat/wms-invoice-agx`); от неё **не** продолжают разработку без `merge` или `rebase` на актуальный `dev`, иначе снова будут конфликты. Новая задача — **новая ветка от свежего `dev`**, один PR в `dev` (как с UI WMS в PR #46).
