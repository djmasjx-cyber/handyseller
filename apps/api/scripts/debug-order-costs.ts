...
/**
 * Отладка: логистика и комиссия по заказу WB.
 * Запуск: ORDER_ID=4501750166 npx ts-node scripts/debug-order-costs.ts
 * Или: EMAIL=user@example.com ORDER_ID=4501750166 npx ts-node scripts/debug-order-costs.ts
 */
...
async function main() {
  ...
  const { data } = await axios.get<Array<Record<string, unknown>>>(
    `${STATISTICS_API}/api/v5/supplier/reportDetailByPeriod`,
    {
