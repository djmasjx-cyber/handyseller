...
const FLOW_STEPS = ["Тара", "Ячейка", "Перемещение", "Размещение"] as const
...
export default function WmsDashboardPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [wRes, lRes, eRes, tRes, meRes] = await Promise.all([
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground leading-relaxed">
            Перемещения и размещение на полку. Накладные, приёмка и штрихкоды — в разделе{" "}
            <Link href="/dashboard/wms/sklad" className="font-medium text-primary underline-offset-4 hover:underline">
              Склад
            </Link>
            .
          </p>
      ...
      >
        ...
        {FLOW_STEPS.map((label, i) => (
          <span key={label} className="inline-flex items-center gap-1">
            ...
            <span className="inline-flex items-center gap-1 rounded-md border bg-background px-2 py-1 font-medium">{label}</span>
          </span>
        ))}
      </div>
        ...
        <CardContent className="space-y-6">
          ...
          {activeWarehouse ? (
            ...
            <p className="text-sm text-amber-800 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
              Выберите склад в списке — без этого ячейки не подгрузятся.
            </p>
          )}
          ...
          <div className="grid gap-4 max-w-xl">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">Ячейка на выбранном выше складе.</p>
              <select
